...
def leapfrog(
        init_pos: np.ndarray[float, Any],
        init_vel: np.ndarray[float, Any],
        init_pos_massive: np.ndarray[float, Any],
        init_vel_massive: np.ndarray[float, Any],
        timestep: int, dt: float,
        masses: np.ndarray[float, Any],
        epsilon: float
) -> tuple[np.ndarray[float, Any], np.ndarray[float, Any], np.ndarray[float, Any], np.ndarray[float, Any]]:
    ...
    for i in range(timestep):
        ...
        vel = velocity[i, :, :] + (gravnb.grav_3body(position[i, :, :],
        ...
        vel = vel + (gravnb.grav_3body(pos, pos_m,
                     masses, epsilon)*dt/2)
        vel_m = vel_m + (gravnb.grav_direct(pos_m,
                         masses, epsilon)*dt/2)
